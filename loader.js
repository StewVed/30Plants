...
*/
var aSWR; //to hold the serviceworker registrations for manually updating later.

function initServiceWorkers() {
  if ('serviceWorker' in navigator) {
    // register the website's own serviceworker to cache it's files.
...
function directCSS(zSrc, fileName) {
  var zCSS = document.createElement('link');
  zCSS.rel = 'stylesheet';
  zCSS.type = 'text/css';
  zCSS.id = fileName + 'l';
  zCSS.href = zSrc;
      zCSS.addEventListener('load', function() {
      this.id = this.id.slice(0, -1);
      filesLoadedCheck();
    });
  document.getElementsByTagName('head')[0].appendChild(zCSS);
}
